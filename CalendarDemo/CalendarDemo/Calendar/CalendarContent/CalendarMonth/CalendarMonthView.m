...
@interface CalendarMonthView ()<CalendarDayViewDelegate>
...
- (void)setupEyeableDayViewArray{
    ...
    for (int i=0; i<self.s_AllDayViewArray.count; i++) {
        CalendarDayView *dayView = self.s_AllDayViewArray[i];
...
- (void)setupPosition{
    ...
    for (int i=0; i<self.s_EyeableDayViewArray.count; i++) {
        CalendarDayView *dayView = self.s_EyeableDayViewArray[i];
